...
**Publication prerequisite remains unresolved:** obtain the intended Recursive Corruption public Google Play developer-account URL from Play Console or a live app listing, open it logged out, verify its identity, then set `googlePlayDeveloperUrl` in `content/site.ts`. A deployment guard prevents publication while this field is unset. Local builds/previews intentionally remain usable. No guessed URL or dead legacy listing is substituted. LinkedIn's prescribed historical profile URL currently opens an authentication wall in the logged-out browser; confirm the profile in an authenticated browser before publishing.
